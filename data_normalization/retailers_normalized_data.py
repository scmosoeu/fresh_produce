...
class Date(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    date_key = db.Column(db.DateTime, nullable=False)#, db.ForeignKey('master_date.date_key'),)

class RetailerPrices(db.Model):

    __tablename__ = 'RetailerPrices'

    RetailerProductID = db.Column(db.Integer, db.ForeignKey('RetailerProducts.id'), nullable=False, primary_key=True)
    Price = db.Column(db.Float, nullable=False)
    DateID = db.Column(db.Integer, db.ForeignKey('Date.id'), nullable=False)
    RetailerID = db.Column(db.Integer, db.ForeignKey("Retailers.id"))
...
#class MasterDate(db.Model):
 ...
 #   calendar_date = db.Column(db.String(20), nullable=False)
  ...
  #  inventory_dates = db.relationship('Inventory', backref='inventory_date')
   #s invoice_dates = db.relationship('Sales', backref='sales_date')
